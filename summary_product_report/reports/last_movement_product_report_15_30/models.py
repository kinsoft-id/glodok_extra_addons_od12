...
class LastMovementProductReport1530(models.Model):
	_name = 'last.movement.product.report.15.30'
	...
	def _select(self):
		query = """
			WITH so AS (
				...
				WHERE so.state in ('done') AND so.date_order BETWEEN (now() - '31 day'::interval) AND (now() - '15 day'::interval)
			), po AS (
				select
					po.id
					,po.date_order
			...
			WHERE p2.days_diff_so BETWEEN 15 AND 31
		"""
		return query
