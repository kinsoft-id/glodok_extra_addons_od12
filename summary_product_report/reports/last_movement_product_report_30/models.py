...
class LastMovementProductReport30(models.Model):
	_name = 'last.movement.product.report.30'
	...
	def _select(self):
		query = """
			WITH so AS (
				...
				WHERE so.state in ('done') AND so.date_order < (now() - '30 day'::interval)
			), po AS (
				select
					po.id
					,po.date_order
